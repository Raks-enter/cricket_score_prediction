...
class DataTransformation:
    ...
    def get_data_transformer_object(self):
        try:
            numerical_columns = ['StudentID', 'Age', 'Absences', 'Tutoring', 'GradeClass']
            ...
            cat_pipeline = Pipeline(steps=[
                ...
                ("scaler", StandardScaler(with_mean=False))  # Avoids issue with sparse matrices
            ])

            logging.info("Categorical and numerical columns processing completed")
    ...
    def initiate_data_transformation(self, train_path, test_path):
        try:
            ...
            if target_column_name not in train_df.columns or target_column_name not in test_df.columns:
                raise CustomException(f"Target column '{target_column_name}' not found in data", sys)

            # Drop target column for input features
